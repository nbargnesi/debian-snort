...
\hline
\texttt{max\_attribute\_hosts} & \texttt{config max\_attribute\_hosts:16384} & Sets a limit on the maximum number of hosts to read from the attribute table.  Minimum value is 8192 and the maximum is 524288 (512k).  The default is 10000.  If the number of hosts in the attribute table exceeds this value, an error is logged and the remainder of the hosts are ignored.  This option is only supported with a Host Attribute Table (see section \ref{targetbased}). \\
\hline
