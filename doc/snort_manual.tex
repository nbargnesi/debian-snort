...
300.  This option can be set from -1 to 1460. It primarily eliminates Snort 
from inspecting larger HTTP Cookies that appear at the end of many client request Headers.
...
This keyword allows values equal to 0 or values greater than or equal to the pattern 
length being searched. The maximum allowed value for this keyword is 65535.
...
There are nine keywords associated with \texttt{http\_encode}. The keywords
...
'base36', 'uencode' and 'bare\_byte' determine the encoding type which would
trigger the alert. These keywords can be combined using a OR operation.
...
\begin{verbatim}
    ...
    http_encode: [uri|header|cookie], [!][<utf8|double_encode|non_ascii|base36|uencode|bare_byte|ascii>];
\end{verbatim}
...
to \emph{=}. Please note that the \emph{!} operator cannot be used with itself.
For example, \emph{!!} is not allowed.
...
\begin{verbatim}
    byte_extract: <bytes_to_extract>, <offset>, <name> \
        [,relative] [,multiplier <multiplier value>] [,big] [,little] [,dce]\ 
        [,string] [,hex] [,dec] [,oct] [,align <align value>] 
\end{verbatim}
...
\hline
\hline
\texttt{content} & \texttt{offset}, \texttt{depth}, \texttt{distance},
\texttt{within}\\
\hline
...
\hline
\texttt{byte\_extract} & \texttt{offset}\\
\hline
